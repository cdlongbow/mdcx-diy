...
项目桌面端原先基于 PyQt5/Qt5，现已完成到 PyQt6 的基线迁移。主要入口和 UI 代码分布在 `main.py`、`mdcx/views`、`mdcx/controllers`、`mdcx/signals.py`、`mdcx/image.py`、`mdcx/config/resources.py` 和少量核心流程弹窗中。这份文档记录迁移时的关键差异、已完成事项和后续界面优化方向。
...
- 已在启动入口按平台应用样式和 palette，减少系统主题对界面观感的干扰。
