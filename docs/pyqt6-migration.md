...
项目桌面端原先用的是 PyQt5/Qt5，现已迁移到 PyQt6。主要入口和 UI 代码在 `main.py`、`mdcx/views`、`mdcx/controllers`、`mdcx/signals.py`、`mdcx/image.py`、`mdcx/config/resources.py` 和少量核心流程弹窗里。以下记录迁移时的关键差别、完成事项和后续界面优化方向。

## PyQt6 主要差别

- `PyQt5` 包名改成了 `PyQt6`
- Qt 的枚举和 Flag 改用标准 Python 的 `Enum`/`Flag`，需要写完整名字，比如 `Qt.AlignmentFlag.AlignCenter`、`QMessageBox.StandardButton.Yes`
- `exec_()` 全部换成 `exec()`
- `QAction` 从 `QtWidgets` 移到 `QtGui`
- Qt6 默认开启高 DPI 缩放，Qt5 里的 `AA_EnableHighDpiScaling`、`AA_UseHighDpiPixmaps` 不再需要
- `pyrcc6` 不再提供，资源文件继续用当前文件路径加载方式
...
- 针对 Windows 8 档界面缩放（跟随系统 / 80% / 90% / 100% / 125% / 150% / 175% / 200%）、macOS Retina、多显示器分别验证
...
- 已修复 PyQt6 短枚举遗漏，包括 `QItemSelectionModel.SelectionFlag.NoUpdate`、`QEvent.Type.Wheel`、`QMessageBox.Icon.Information`
