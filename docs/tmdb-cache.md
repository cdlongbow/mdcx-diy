...
1. 用户刮削一部影片
2. 系统发现某个演员在 Excel 里没有 TMDB ID
3. 先用 Excel 已有数据（中文名、别名等）翻一翻
4. 找不到就去 TMDB 官网查（3 路并发，限流 3.5 req/s）
5. TMDB 找到后，优先用它的 `original_name`（通常是日文原名）填到 Excel 的"日文名"列
6. 填好 TMDB ID，自动刷新缓存

### 场景 4：自动补充演员网址

每次 TMDB 查完后，系统会统一检查哪些演员有 TMDB ID 但没有网址链接：

1. 从查询结果里挑出"有 TMDB ID，但缺网址"的演员
2. 去 Excel 找到这个演员的日文原名
3. 用日文原名去 LibreDMM 搜他的个人主页
4. 搜到了就填回 Excel 的"信息链接"列
5. 整个过程集中在 TMDB 查询的末尾一步完成，不会中途反复打断
...
TMDB 查演员时，会返回一个 `original_name` 字段（一般是日文原名，比如"三上悠亜"），这个字段优先用于：

- **填 Excel 表**：作为"日文原名"列写进去，而不是用搜索时输入的中文名
- **搜演员网址**：先反查 Excel 找到日文原名，再用原名去 LibreDMM 搜，搜索命中率更高
- **万一没有**：如果 TMDB 没返回 `original_name`，就用搜索用的名字
