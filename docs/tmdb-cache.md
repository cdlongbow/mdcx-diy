...
MDCx 的 TMDB 缓存系统是一个基于 Excel 文件的持久化 ID 映射数据库，用于缓存演员名到 TMDB ID 的对应关系，避免重复查询 TMDB API。
...
3. 直接返回缓存的 TMDB ID，无需网络请求

### 自动补充演员网址

每次 TMDB 查询完成后，系统检查哪些演员有 TMDB ID 但没有信息链接：

1. 从查询结果中筛选出"有 TMDB ID 但缺网址"的演员
2. 在 Excel 中找到该演员的日文原名
3. 用日文原名去 LibreDMM 搜索其个人主页
4. 搜到后填回 Excel 的"信息链接"列
5. 整个过程集中在 TMDB 查询末尾一步完成，不会中途反复打断
...
TMDB 查演员时返回的 `original_name` 字段（一般是日文原名，如"三上悠亜"），优先用于：

- **填 Excel 表**：作为"日文原名"列写进去，而不是用搜索时输入的中文名
- **搜演员网址**：先反查 Excel 找到日文原名，再用原名去 LibreDMM 搜索
- **回退**：如果 TMDB 未返回 `original_name`，使用搜索时用的名字
...
TMDB 请求可以通过"走代理网站"功能配置是否走代理：

- 走代理网站列表为空时，TMDB 请求默认直连
- 如需 TMDB 走代理，将 `api.tmdb.org` 加入走代理网站列表
...
1. **维护演员数据库**：尽量保持演员日文名、别名和 TMDB ID 准确
