...
**A**: 大部分配置即时生效，少数需要重启。缩放比例修改后必须重启软件才能生效。

### Q: 界面太大或太小怎么办？

**A**: 在"设置" -> "界面外观"中找到"高分屏缩放"区域，通过缩放比例下拉框选择合适的比例（跟随系统 / 80% / 90% / 100% / 125% / 150% / 175% / 200%）。保存配置后重启软件即可生效。如果仍然不合适，可以配合"非整数倍缩放"开关一起使用，或手动拖拽调整窗口大小。
