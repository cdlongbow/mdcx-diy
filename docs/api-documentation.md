...
本文档介绍 MDCx 核心模块的 API。如果你刚接触项目，可以先看各模块的"简单说"部分。
...
主刮削器类，负责协调整个刮削流程。它会调度爬虫、收集数据、生成结果。
...
**参数**：
- `crawler_provider`：爬虫提供器实例（告诉刮削器去哪里拿数据）
...
文件爬虫类，负责调用各个爬虫获取数据。它知道去哪些网站找信息，以及怎么把信息拼在一起。

简单说：这是"找数据的人"——它会识别影片、调用多个爬虫、汇总结果。
...
识别番号和刮削类型。比如输入一个文件名，它先判断这是什么类型的影片。
...
按字段优先级调用多个爬虫。不同网站数据质量不同，这个方法按优先级从好到差依次尝试。
...
NFO 文件是一种元数据文件，播放器（如 Kodi、Jellyfin）用它来展示影片信息。这个模块负责生成和读取 NFO。

简单说：NFO 就是影片的"身份证"文件。
...
TMDB（The Movie Database）是一个公开的电影数据库。这个模块用来查演员信息。

简单说：帮你查演员是谁、来自哪里、以及他们的 TMDB 编号。
...
配置系统管理所有设置——爬哪些网站、用什么语言、字段优先级等。

简单说：这个模块管"刮削器怎么工作"——用哪个网站、显示什么语言、文件怎么命名。
...
命名系统决定刮削后的文件叫什么名字。它使用模板来生成文件名。

简单说：你告诉它"文件名长什么样"，它就按那个样子生成。
