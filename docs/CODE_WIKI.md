...
MDCx（Movie Data Capture X）是一个视频元数据刮削和管理工具，从 45+ 个成人视频网站获取元数据（标题、简介、演员、标签、封面图等），支持与 Emby、Jellyfin 集成（当前已注册 45 个爬虫，下拉框由已注册爬虫动态生成）。

### 核心功能

1. **智能刮削**：45 个数据源网站，自动识别番号和类型
...
│   │   ├── utils.py                # 工具函数
│   │   └── web.py                  # Web 操作
│   ├── crawlers/                   # 爬虫实现（45 个已注册站点，下拉框动态生成）
│   ├── gen/                        # 自动生成的枚举
│   ├── models/                     # 数据模型
│   ├── tools/                      # 工具模块
│   │   ├── actress_db.py           # 演员数据库
...
- `Website`：47 个站点枚举（下拉框由已注册的 45 个爬虫动态生成）
...
每个网站一个爬虫文件，共 45 个已注册站点（下拉框动态生成）
...
**已迁移站点**：
dmm, javdb, dmm_api, javdb_app, javdb_api, avbase, missav, faleno, jav321, cableav, madouqu, mmtv, dahlia, fantastica, avsox, cnmdb, hscangku, kin8, lulubar, xcity, giga, mdtv, mgstage, javday, fc2ppvdb, prestige, fc2club, fc2, fc2hub, javbus, freejavbt, hdouban, iqqtv, airav_cc, getchu, getchu_dmm, mywife, javlibrary, official, theporndb, r18dev
