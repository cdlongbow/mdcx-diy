...
1. **智能刮削**：支持 42+ 个数据源网站，自动识别番号和马赛克类型
2. **元数据管理**：生成符合 Kodi/Emby 规范的 NFO 文件（30+ 字段）
3. **图片处理**：自动下载、裁剪、添加水印
4. **翻译功能**：支持 Google/百度/DeepL/LLM 翻译
5. **命名管理**：灵活的 Jinja2 模板系统，支持自定义命名规则
6. **Amazon 集成**：条码识别，自动匹配高清封面
7. **异步处理**：高效的并发刮削能力
8. **演员数据库**：Excel 格式的演员信息管理，支持别名、多语言名、TMDB ID、信息链接
9. **TMDB 集成**：演员 TMDB ID 自动查询和补全，`original_name` 优先作为日文原名
10. **LibreDMM 自动补链**：刮削完成后统一扫描缺链接演员，自动补全信息链接
11. **Emby 集成**：自动更新 Emby/Jellyfin 媒体服务器
