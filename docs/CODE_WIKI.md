...
MDCx（Movie Data Capture X）是一个现代化的视频元数据刮削和管理工具，主要用于从 42+ 个成人视频网站获取元数据信息，包括标题、简介、演员、标签、封面图等，并支持与 Emby、Jellyfin 等媒体服务器集成。

### 核心功能

1. **智能刮削**：支持 42+ 个数据源网站，自动识别番号和类型
2. **元数据管理**：生成 NFO 文件（30+ 个字段），Kodi/Emby/Jellyfin 通用
3. **图片处理**：自动下载、裁剪、加水印
4. **翻译功能**：支持 Google/百度/DeepL/LLM 五引擎
5. **命名管理**：Jinja2 模板，想怎么命名都行
6. **Amazon 集成**：扫条码自动匹配高清封面
7. **异步处理**：多个任务一起刮，不卡
8. **演员数据库**：Excel 表管演员，支持别名、多语言名、TMDB ID、网址
9. **TMDB 集成**：自动查演员的 TMDB ID，日文名用 TMDB 原版名字
10. **自动补网址**：刮完统一查缺链接的演员，用日文名去 LibreDMM 补上
11. **Emby 集成**：自动更新 Emby/Jellyfin 媒体服务器信息
...
2. **获取 TMDB ID**
   - 刮削流程调用 `fetch_actor_tmdb_ids()` 查询缺失的 TMDB ID
   - 使用令牌桶限流器控制请求速率
   - Semaphore 限制并发数为 3
...
- **演员数据库缓存**：程序启动时加载到内存
- **TMDB ID 映射缓存**：已写入演员数据库的 TMDB ID 会在后续刮削中直接命中
...
1. **维护数据库**：保持演员日文名、别名和 TMDB ID 准确，提高缓存命中率
