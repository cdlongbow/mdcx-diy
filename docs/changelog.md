...
## v2.0.0 (2026-07-12) 重大更新版本

### 版本说明

v2.0.0 是本次重大版本号升级，包含自 v1.4.0 以来的所有累积改进
...
- **内置 CF Bypass（零配置）**：新增"启用内置 Bypass"选项，勾选后 MDCx 自动在后台启动本地旁路服务（基于隐身浏览器），无需手动搭建外部服务。配套设置界面复选框
- **新增配置项**：`cf_bypass_auto`（bool，默认 false），与外部 `cf_bypass_url` 互斥，地址为空时自动启用本地服务
- **新增MissAV免防护墙爬虫 missav_api**：原MissAV爬虫常被防护墙挡住；现在多了一条免防护墙通道，不用费力绕墙也能直接刮到它的影片信息
